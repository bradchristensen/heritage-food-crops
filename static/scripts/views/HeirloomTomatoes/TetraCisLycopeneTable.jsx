...
import tomatoTable from './TetraCisLycopeneTableData';
...
function FruitCharacteristicsTable({ clientWidth }) {
    return (
        <table
        ...
        >
            <thead>
                <tr>
                    <th>Size:</th>
                    <th>{clientWidth > 800 ? 'Description:' : 'Desc:'}</th>
...
class TetraCisLycopeneTable extends PureComponent {
    ...
    cellRenderer({
        ...
        let content = tomatoTable[rowIndex][dataKey];
        if (this.state.clientWidth <= 1280 && dataKey === 'source') {
            const url = tomatoTable[rowIndex].sourceUrl;
            content = url ? <a href={url} {...targetBlank}>Link</a> : null;
        }
        if (dataKey === 'characteristics') {
            content = (
                <Fragment>
                    <strong>Plant:</strong> {tomatoTable[rowIndex].plantCharacteristics}<br />
                    <strong>Fruit:</strong> {tomatoTable[rowIndex].fruitCharacteristics}
                </Fragment>
            );
        }
                        ...
                        {({ width }) => (
                            <div ref={registerChild}>
                                <Table
                                    ...
                                    overscanRowCount={this.state.clientWidth > 800 ? tomatoTable.length : 10}
                                    rowClassName={({ index }) => (index % 2 ? 'row-even' : 'row-odd')}
                                    rowCount={tomatoTable.length}
                                    rowGetter={({ index }) => tomatoTable[index]}
