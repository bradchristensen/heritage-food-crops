...
router.post('/competition', parse, async (req, res) => {
    const ipAddress = req.headers['x-forwarded-for'] || req.connection.remoteAddress;
    const userAgent = req.headers['user-agent'];

    const toRender = (
        <div>
            <p style={{ fontSize: '24px' }}>{req.body.entry}</p>
            <p>
                <small>
                    <em>
    ...
    const bodyPlainText = `${req.body.entry}
...
`;
    ...
    const message = {
        ...
        content: {
            ...
            reply_to: `Competition entrant <${req.body.email}>`,
            subject: 'HFCRT - Naming competition entry',
            html: bodyHtml,
            text: bodyPlainText,
        },
    };
