...
export default class ScatterPoint extends PureComponent {
  render() {
    const { datum, scale, size, style: rawStyle, x, y } = this.props;

    const rectSize = 3;

    // These unused vars intentionally extracted from `style`
    // eslint-disable-next-line no-unused-vars
    const { _x1, width, ...style } = rawStyle;

    const lineStyle = {
      stroke: style.stroke === "transparent" ? style.fill : style.stroke,
