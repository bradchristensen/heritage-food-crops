...
const TICK_FORMAT = val => val.toFixed(1);
const LABEL_FORMATTER = dataPoint => `y: ${dataPoint.y}`;
  ...
  render() {
    ...
    const chart = (
      <VictoryChart
        height={HEIGHT}
        width={WIDTH}
        containerComponent={<VictoryVoronoiContainer theme={VictoryTheme} />}
        theme={VictoryTheme}
      >
        <VictoryLabel
          dx={WIDTH / 2}
    ...
    return (
      <Container>
        <div style={{ width: "100%" }}>
          <div style={{ maxHeight: "600px" }}>{chart}</div>
          ...
          >
            <strong>
              7 days of treatment IC<sub>50</sub> # 40μg/ml
            </strong>
            <br />
            <em>
              The IC<sub>50</sub> is{" "}
              <a href="https://simple.wikipedia.org/wiki/IC50" {...targetBlank}>
                the measurement of the concentration needed to kill{" "}
                <nobr>half the cancer cells</nobr>
              </a>
              . <nobr>The lower the number,</nobr> <nobr>the better</nobr>{" "}
              (meaning that more cells are killed at a lower concentration).
            </em>
            <br />
            <small>
              Note: this interactive graph has been generated based on{" "}
              <a
                href={require("../../../images/layout/montys-surprise/graph-cancer-cell-proliferation.png")}
              >
                the original bitmap image
              </a>
              . <nobr>Data points are estimates.</nobr>
            </small>
          </div>
        </div>
      </Container>
